...
genelist <- readRDS("~/Fig3_genomics_normal_cells/copy_number/geneCNAlist.Rds")
cnlist <- readRDS("~/Fig3_genomics_normal_cells/copy_number/cnlist.Rds")

paths <- system("realpath ~/Sequencing_updated_reference/CA*/*/purple/*purity.tsv", intern = T)
...
sapply(seq_along(genelist), function(x){
  ...
  pdf(paste0("~/Fig3_genomics_normal_cells/copy_number/purple_fig/", names(genelist)[x], ".pdf"), width = 10 , height= 2.7)
...
joint_cn <- readRDS("~/240628_ATAClone/CA0027/dura_base_skull_13/joint_cn.Rds")
