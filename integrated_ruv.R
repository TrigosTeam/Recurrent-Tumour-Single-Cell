...
test <- readRDS("~/integration/2024_06/tumor_only_RNA_simspec_srt.Rds")
samples <- test$sample
...
saveRDS(logPAC, "~/integration/2024_06/tumor_only_logPAC.Rds")
...
test$site <- sites
saveRDS(test, "~/integration/2024_06/tumor_only_ruvsrt.Rds")

pdf("~/integration/2024_06/tumor_only_ruv_srtplots.pdf", width = 12, height = 10)
...
# tumor normal ------------------------------------------------------------
#15/07/2024
test <- readRDS("~/integration/2024_06/tumor_normal_RNA_simspec_srt.Rds")
...
saveRDS(test, "~/integration/2024_06/tumor_normal_RNA_simspec_newsrt.Rds")
...
pdf( "~/integration/2024_06/tumor_normal_RNA_simspec_newUMAP.pdf", width = 20, height = 10)
...
saveRDS(logPAC, "~/integration/2024_06/tumor_normal_logPAC.Rds")

rm( list = "ruv3nb_out")
gc()

logPAC <- readRDS( "~/integration/2024_06/tumor_normal_logPAC.Rds")
test[["ruv3"]]<- CreateAssayObject(data = logPAC)
DefaultAssay(test) <-"ruv3"
test[["ruv3"]]@scale.data <- as.matrix(test[["ruv3"]]@data)
test <- FindVariableFeatures(test)
...
sites[grep("bladder", sites)] <- "bladder"
test$site <- sites
saveRDS(test, "~/integration/2024_06/tumor_normal_ruvsrt.Rds")

pdf("~/integration/2024_06/tumor_normal_ruv_srtplots.pdf", width = 12, height = 10)
DimPlot(test, label = T, raster = F)
DimPlot(test, group.by = "patient", raster = F, label = T, repel = T) +NoLegend()
