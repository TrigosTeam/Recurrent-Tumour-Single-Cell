...
library(patchwork)
library(ggplot2)
paths <- system("realpath ~/230113_seurat_intron/*/*/*doublets_removed.Rds", intern = T)
paths_split <- unlist(lapply(strsplit(paths, split = "/"), function(x) paste(x[6], x[7], sep = "_")))
paths2 <- system("realpath ~/240603_seurat_intron/CA0027/dura*/*doublets_removed_v4.Rds", intern = T)
...
lapply(split(paths, patients), function(pathp){
  ...
  saveRDS(test, paste0("~/Fig2_signature/sc_integration/perpatient/tumor_normal/",patient, "_tumor_normal_RNA_simspec_srt.Rds"))
  ...
  pdf( paste0("~/Fig2_signature/sc_integration/perpatient/tumor_normal/",patient, "_tumor_normal_RNA_simspec_UMAP.pdf"), width = 10, height = 5)
