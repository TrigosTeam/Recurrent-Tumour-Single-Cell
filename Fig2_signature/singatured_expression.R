...
library(ggpubr)
library(stringr)
load("~/Analysis/signatures_analysis/objects/all_gene_sets.rda")
source("~/functions.R")
...
pdf("~/Fig2_signature/signature_tumor_only.pdf", width = 6, height = 5)
...
for (p in paths_split){
  ...
  saveRDS(perp_srt@meta.data, paste0("~/Fig2_signature/perpatient_signature_tumor_only/",p, "_meta.Rds"))
  pdf(paste0("~/Fig2_signature/perpatient_signature_tumor_only/",p, "_signature.pdf"), width = 4, height = 3)
  for (i in names(temp)){
...
for(p in paths_split){
  perp_srt <- readRDS(paths[p])
  perp_srt <- AddMetaData(perp_srt, metadata = readRDS(paste0("~/Fig2_signature/perpatient_signature_tumor_only/",p, "_meta.Rds")))
  ...
  # saveRDS(tot, paste0("~/Fig2_signature/plot_obj/", p, ".Rds"))
...
paths <- system("realpath ~/Analysis/230113_seurat_intron/*/*/*non_tumour_removed.Rds", intern = T)
paths_split <- unlist(lapply(strsplit(paths, split = "/"), function(x) paste(x[6], x[7], sep = "_")))
paths2 <- system("realpath ~/Analysis/240603_seurat_intron/CA0027/dura*/*non_tumour_removed_v4.Rds", intern = T)
...
for(i in paths_split){
 ...
 set.seed(100)
  ...
  saveRDS(srt@meta.data, paste0("~/Fig2_signature/persample_signature/",i, "_meta.Rds"))
  pdf(paste0("~/Fig2_signature/persample_signature/",i, ".pdf"), width = 4, height = 3)
  for (n in names(temp)){
...
f1 +f2+f3+f4+f5+f6 + plot_layout(ncol = 3, nrow = 2, byrow = F)
ggsave(paste0("~/Fig2_signature/pdf/",i, "_signatures.pdf"),  width = 9.5, height = 4)


# saveRDS(g2, paste0("~/Fig2_signature/plot_obj/", i, ".Rds"))
# png(paste0("~/Fig2_signature/png/", i, ".png"), width = 700, height = 300)
# print(g2)
# dev.off()
