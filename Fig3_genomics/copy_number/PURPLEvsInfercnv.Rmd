...
for (i in names(genelist)[names(genelist)%in%list.files("~/CASCADEpaper/paper/Fig3/Infercnv/infercnv_run/clustered")]){
...
mat <- temp@expr.data[,anno_t$Cell]
...
print(p2)
  ...
  print(g)
  pdf(paste0("~/CASCADEpaper/paper/Fig3/copy_number/svg/", i, ".pdf"), width = 12, height = 10)
  # print(g)
  gridExtra::grid.arrange(p, p2, g, nrow = 3)
  dev.off()
}
```
mark significant regions in purple region plot 
```{r eval=FALSE, fig.height=3, fig.width=12, include=FALSE}
for (i in names(genelist)[names(genelist)%in%list.files("~/CN/rerun")]){
  temp <- readRDS(paste0("~/CN/rerun/", i, "/preliminary.infercnv_obj"))
anno <- read.table(paste0("~/CN/rerun/", i, "_annotation.txt"), header = F, sep = "\t")
colnames(anno) <- c("Cell", "ID")
anno_t <- anno[grep("cluster", anno$ID), ]
mat <- temp@expr.data
mat <- mat[,colnames(mat)%in%anno_t$Cell]

cell_ord_uncluster <- read.dendrogram(paste0("~/CN/rerun/", i, "_test/infercnv.preliminary.observations_dendrogram.txt"))
cell_ord <-list()
hc <- temp@tumor_subclusters$hc

for (n in grep("cluster",names(hc),value = T)){
  test <- data.frame(cell = hc[[n]]$labels, order =hc[[n]]$order, cluster = n)
  cell_ord[[n]]<- test
}

cell_ord <- do.call(rbind.data.frame, cell_ord)
cell_ord <- data.table(cell_ord, key = "cell")
gene_ord<- temp@gene_order
gene_ord$width <- gene_ord$stop-gene_ord$start
gene_ord$gene <- rownames(gene_ord)
...
gene_ord<- do.call(rbind.data.frame, df_list)
gene_ord_f <- gene_ord[,c("gene",'chr', "width", "start")]
gene_ord_f <- gene_ord_f[gene_ord_f$gene%in%genelist[[i]]$gene, ]

 genelist[[i]] <- genelist[[i]][genelist[[i]]$gene%in%gene_ord_f$gene,]
  genes <- genelist[[i]]
  genes$region <- paste(genes$chrom, genes$f_start, genes$f_end, sep = ";")
  genes$yes_no <- genes$region%in%sig_region_2[[i]]
g <- ggplot(data =genes,aes(x = seg, y = samples, fill = yes_no, width = log10(bin_width)))+
  geom_tile()+
  facet_grid(.~chrom, scale = "free_x")+
  theme(strip.text.x = element_text(size = 10), axis.title = element_blank(), 
        axis.ticks.x = element_blank(),
        ...
        panel.grid = element_blank())+ ggtitle(paste("PURPLE plot:",i))+
  scale_fill_manual(values = c("FALSE" = "transparent", "TRUE"="black"))
  print(g)
  
  ggsave(paste0(i, "purple_sig_region.svg"), width = 12, height = 3, units = "cm", bg = "transparent")
