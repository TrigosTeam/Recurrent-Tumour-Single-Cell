...
geneCNA <- readRDS("~/CASCADEpaper/paper/Fig3/copy_number/geneCNAlist.Rds")
...
SO_table <- readRDS("~/CASCADEpaper/paper/Fig3/point_mutation/point_mutation_table_strelka.Rds")
```
...
wgd <- unlist(lapply(wgd, function(x) return(x$wholeGenomeDuplication)))
wgd <- setNames(wgd, paste(ids, sites, sep = "_"))
```

## Viriant class matrix 
```{r point mutation, message=FALSE, warning=FALSE, include=FALSE}
tmb <- c()
for (i in names(SO_table)){
  patient <- substr(i, 1, 6)
  sample <- substring(i, 8)
  temp <- read_tsv(list.files(paste0("/trigos_team/CASCADE/Sequencing_updated_reference/", patient, "/", sample, "/purple"), full.names = T, pattern = "purity.tsv"), col_names = T)
  print(i)
  # print(temp$wholeGenomeDuplication)
  # print(nrow(SO_table[[i]])/3299)
  # print(temp$tmbPerMb)
  tmb <- c(tmb, temp$tmbPerMb)
  #print(table(SO_table[[i]]$VARIANT_CLASS)/3299*temp$tmbPerMb)
}
...
for(i in rownames(CN.mat)){
  for( n in colnames(CN.mat)){
    ...
    if(nrow(cons)>0){
      SM.mat[i, n] <- paste(SM.mat[i,n], unique(cons$Consequence), sep = ";")
    }
  }
}

SO_col <- c("deeppink", "orangered", "limegreen", "tan1", "deepskyblue")
names(SO_col) <- unique(f_SO$Consequence)
...
onc <- oncoPrint(mat2, alter_fun = alter_fun, name = "part1", 
          ...
          row_order =  gene_order, column_names_rot = -90, column_order = sample_order,
...
pdf("~/CASCADEpaper/paper/Fig3/oncoplot.pdf", width = 10, height = 11)
