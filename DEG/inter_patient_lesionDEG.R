...
# saveRDS(markers, "~/integration/2024_06/DEG/inter_patient_DEG.Rds")
# rm(list = ls())
# gc()
# 
...
#   # names(markerlist) <- sapply(groups, function(x) paste(x, collapse = "-"))
#   markers <- FindAllMarkers(srt, test.use = "MAST")
#   saveRDS(markers, paste0("~/integration/2024_06/DEG/inter_lesion/", i, ".Rds"))
#   print(i)
# }
...
Idents(srt) <- srt$site
markers <- FindAllMarkers(srt, test.use = "MAST")
saveRDS(markers, "~/integration/2024_06/DEG/inter_organ_DEG.Rds")
