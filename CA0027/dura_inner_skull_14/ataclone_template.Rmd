...
```{r}
source("~/CASCADE/Analysis/240628_ATAClone/scripts/make_peaks_altius.R")
source("~/CASCADE/Analysis/240628_ATAClone/scripts/ataclone_binning.R")
source("~/CASCADE/Analysis/240628_ATAClone/scripts/ataclone_cn.R")
```
...
```{r}
id <- "CA0027"
site <- "dura_inner_skull_14"
override_purple_path <- NULL
override_frag_path <- "~/CASCADE/SingleCell/CA0027/rerun/dura_inner_skull_14/CA0027_dura_inner_skull_14_multi/outs/atac_fragments.tsv.gz"
override_srt_path <- "~/CASCADE/Analysis/240603_seurat_intron/CA0027/dura_inner_skull_14/CA0027_dura_inner_skull_14_srt_new_filtering_doublets_removed.Rds"
override_cr_arc_barcode_stats_path <- "~/CASCADE/SingleCell/CA0027/rerun/dura_inner_skull_14/CA0027_dura_inner_skull_14_multi/outs/per_barcode_metrics.csv"
...
if (is.null(override_purple_path)){
  purple_path <- paste0("~/CASCADE/Sequencing_updated_reference/", id, "/", site, "/purple/", id, "_", site, ".purple.cnv.somatic.tsv")
} else {
  purple_path <- override_purple_path
}

if (is.null(override_frag_path)){
  frag_path <- paste0("~/CASCADE/SingleCell/", id, "/", site, "/multiome/", id, "_", site, "_multi/outs/atac_fragments.tsv.gz")
} else {
  frag_path <- override_frag_path
}

if (is.null(override_srt_path)){
  srt_path <- paste0("~/CASCADE/Analysis/230113_seurat_intron/", id, "/", site, "/", id, "_", site, "_srt_new_filtering_doublets_removed.Rds")
} else {
  srt_path <- override_srt_path
}

if (is.null(override_cr_arc_barcode_stats_path)){
  cr_arc_barcode_stats_path <- paste0("~/CASCADE/SingleCell/", id, "/", site, "/multiome/", id, "_", site, "_multi/outs/per_barcode_metrics.csv")
} else {
  cr_arc_barcode_stats_path <- override_cr_arc_barcode_stats_path
}

if (is.null(override_cr_atac_barcode_stats_path)){
  cr_atac_barcode_stats_path <- paste0("~/CASCADE/SingleCell/", id, "/", site, "/ATACseq/", id, "_", site, "_atac/outs/singlecell.csv")
} else {
...
```{r}
atac_peaks <- get_peaks_altius("~/CASCADE/Analysis/240628_ATAClone/files/dat_bin_FDR01_hg38.mtx.gz?download=1", "~/CASCADE/Analysis/240628_ATAClone/files/DHS_Index_and_Vocabulary_metadata.tsv?download=1", "~/CASCADE/Analysis/240628_ATAClone/files/DHS_Index_and_Vocabulary_hg38_WM20190703.txt.gz?download=1", 15)
...
```{r}
pseudo_bulk_clusters <- peak_counts_leiden
pseudo_bulk_clusters <- ifelse(pseudo_bulk_clusters == ref.cluster, 2, 1)
joint_cn_estimates_pseudobulk <- get_copy_number(peak_counts_filtered, pseudo_bulk_clusters, 2, joint = T)
purple_cn_estimates <- get_purple_cn(purple_path, rownames(peak_counts_filtered))

ataclone_purple_cor <- cor(purple_cn_estimates, joint_cn_estimates[,which(pseudo_bulk_clusters == 1)[1]], use = "pairwise.complete.obs")
plot(purple_cn_estimates, joint_cn_estimates[,which(pseudo_bulk_clusters == 1)[1]], xlab = "purple.cn", ylab = "ataclone.cn", main = paste0("cor = ", ataclone_purple_cor))
```
