...
# tumor normal --------------------------------------------------------------
paths <- system("realpath ~/integration/2024_06/perpatient/tumor_normal/*simspec_srt.Rds", intern = T)
...
#saveRDS(ruv3nb_out, paste0("~/integration/2024_06/perpatient/tumor_normal/", i, "_ruvout.Rds"))
...
saveRDS(logPAC, paste0("~/integration/2024_06/perpatient/tumor_normal/", i, "_logPAC.Rds"))
...
test$site <- sites
saveRDS(test, paste0("~/integration/2024_06/perpatient/tumor_normal/", i, "_ruvsrt.Rds"))

pdf(paste0("~/integration/2024_06/perpatient/tumor_normal/", i, "_ruvUMAP.pdf"), width = 12, height = 10)
...
paths <- system("realpath ~/integration/2024_06/perpatient/tumor_only/*simspec_srt.Rds", intern = T)
patients <- substr(unlist(lapply(strsplit(paths, split = "/"), function(x) x[8])), 1, 6)
paths <- setNames(paths, patients)
...
for (i in patients){
  ...
  #saveRDS(ruv3nb_out, paste0("~/integration/2024_06/perpatient/tumor_only/", i, "_ruvout.Rds"))
  print("tumor only ruv finished")
  ...
  saveRDS(logPAC, paste0("~/integration/2024_06/perpatient/tumor_only/", i, "_logPAC.Rds"))
  ...
  sites[grep("bladder", sites)] <- "bladder"
  test$site <- sites
  saveRDS(test, paste0("~/integration/2024_06/perpatient/tumor_only/", i, "_ruvsrt.Rds"))
  
  pdf(paste0("~/integration/2024_06/perpatient/tumor_only/", i, "_ruvUMAP.pdf"), width = 12, height = 10)
  p <- DimPlot(test, label = T, raster = F)
  print(p)
  p <- DimPlot(test, group.by = "patient", raster = F, label = T, repel = T) +NoLegend()
  print(p)
