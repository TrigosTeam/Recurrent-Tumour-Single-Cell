...
setwd(~/Fig4-5_archetype")
marker_cluster <- readRDS(~/Fig4-5_archetype/clean_module.Rds")
...
for(i in paths_split){
  ...
  saveRDS(srt@meta.data[, grep("Module", colnames(srt@meta.data))], paste0(~/Fig4-5_archetype/4_srt_meta/", i, ".Rds"))
  
  pdf(paste0(~/Fig4-5_archetype/4_srt_plot/", i, ".pdf"), width = 12, height = 6)
  ...
  saveRDS(cells, paste0(~/Fig4-5_archetype/4_population/", i, "_cell.Rds") )
  ...
  saveRDS(cepoout, paste0(~/Fig4-5_archetype/4_markers/", i, "_marker.Rds") )
  print(i)
}
...
marker <- list.files(~/Fig4-5_archetype/4_markers/", pattern = "_marker.Rds", full.names = T) %>% lapply(., readRDS)
DEG <- lapply(marker, lapply, function(x) x$DEG )
cepo <- lapply(marker, lapply, function(x) x$cepo )
names(DEG) <- list.files(~/Fig4-5_archetype/4_markers/", pattern = "_marker.Rds") %>% gsub("_marker.Rds", "", .)
...
cells <- list.files(~/Fig4-5_archetype/4_population", pattern = "_cell.Rds", full.names = T) %>% lapply(., readRDS)
names(cells) <- list.files(~/Fig4-5_archetype/4_population", pattern = "_cell.Rds") %>% gsub("_cell.Rds", "",.)
...
# combine plots -----------------------------------------------------------
library(stringr)
enlist <- readRDS("~/Fig4-5_archetype/4_enlist.Rds")
endf <- lapply(enlist, lapply, lapply, function(x) if(class(x)!= "try-error") x@result)
