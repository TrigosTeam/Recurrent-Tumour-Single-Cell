...
```{r}
meta2 <- meta
df <- meta %>% group_by(site, group) %>% summarise(n = n()) %>% mutate(freq = n/sum(n))

df$group  <- gsub("_", "", df$group)
colnames(df) <- c("site", "module", "n","freq")
df$labs <- paste0(signif(df$freq*100, 2), "%")
df$labs[df$freq < 0.03] <- NA
g <- ggplot(df, aes(x = site, y = freq, fill =module)) + geom_bar(stat = "identity") +
    geom_text(aes(label = labs),
            position = position_stack(vjust = 0.5),        # Position text in the middle of the bars
            color = "white",                               # Set text color
            size = 4)+
  ...
  theme_classic(base_size = 18)+
  theme(axis.text.x = element_text(hjust = 1, vjust = 0.5, angle = 90), legend.position = "none")+
  labs(x = "Site", y = "Cell Proportion")

meta2$group[grep("&", meta2$group)] <- "transition"
df <- meta2 %>% group_by(site, group) %>% summarise(n = n()) %>% mutate(freq = n/sum(n))
df$group  <- gsub(" high", "", df$group)
colnames(df) <- c("site", "module", "n","freq")
colors2 <- c(color_high, setNames(c("lightgrey", "slategrey"), c("low expression", "transition")))
names(colors2)[1:6] <- names(final_signature)
df$labs <- paste0(signif(df$freq*100, 2), "%")
df$labs[df$freq < 0.03] <- NA


g2 <- ggplot(df, aes(x = site, y = freq, fill =module)) + geom_bar(stat = "identity") + 
  geom_text(aes(label = labs),
            position = position_stack(vjust = 0.5),        # Position text in the middle of the bars
            color = "white",                               # Set text color
            size = 4)+
  ...
  theme(axis.text.x = element_text(hjust = 1, vjust = 0.5, angle = 90), legend.position = "none")+
  labs(x = "Site", y = "Cell Proportion")
p <- g/g2 + plot_layout(guides= "collect", axes = "collect") 
# ggsave("pdf/expression_moudle_bysite.pdf", width = 7, height = 5)
p
```


```{r legends, fig.height=6, fig.width=6}
df <- moduledf
p1 <- ggplot(df[grepl("high|low expression", df$module), ], aes(x = sample, y = freq, fill =module)) + geom_bar(stat = "identity") + 
  ...
  scale_fill_manual(values = colors, name = "Module", guide = guide_legend(nrow = 2), labels = c(paste(new_name, "high"),   "low expression")) + 
...
df2 <- df2 %>% mutate(tile1 = ordered(ntile(n1, 5)),
                      tile2 =  ordered(ntile(n2, 5)),
                      tile3 =  ordered(ntile(n3, 5)),
                      tile4 =  ordered(ntile(n4, 5)),
                      tile5 =  ordered(ntile(n5, 5)),
...
temp1 <- reshape2::melt(df2[,c(1:6, 13)], id.vars = "sample", variable.name = "n", value.name = "prop")
temp1 <- temp1 %>% group_by(n) %>% mutate(tile = ordered(ntile(prop, 5)))
temp1$n <- as.character(temp1$n)
for (i in 1:6){
  temp1$n[temp1$n == paste0("n", i)] <- new_name[i]
