...
# paths <- system("realpath /trigos_team/CASCADE/Analysis/230113_seurat_intron/*/*/*doublets_removed.Rds", intern = T)
# paths_split <- unlist(lapply(strsplit(paths, split = "/"), function(x) paste(x[6], x[7], sep = "_")))
# paths2 <- system("realpath /trigos_team/CASCADE/Analysis/240603_seurat_intron/CA0027/dura*/*doublets_removed_v4.Rds", intern = T)
...
# saveRDS(test, "~/integration/2024_06/tumor_normal_RNA_simspec_srt.Rds")
...
# pdf( "~/integration/2024_06/tumor_normal_RNA_simspec_UMAP.pdf", width = 20, height = 10)
...
test <- subset(test, cells = colnames(final_normalsrt))
DefaultAssay(test) <- "RNA"
lib.gmean3 <- exp(mean(log(colSums(test[["RNA"]]@counts))))
target_pseudocount = 1
test <- test %>%
