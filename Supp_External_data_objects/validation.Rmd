...
setwd("~/Supp_External_data_objects")
library(Seurat)
library(DESeq2)
...
## GSE126078
```{r}
# setwd("~/Supp_External_data_objects/GSE126078")
...
mol_prof_specimen <- readRDS("~/Supp_External_data_objects/mol_prof_specimen.Rds")
counts[1:3, 1:3]
class(counts)
...
g1 <- ggplot(df, aes(x = group, y = exp, fill = group))+ 
  geom_violin(aes( color = group))+ geom_boxplot(width=0.1, outlier.size = 2) + theme_bw(base_size = 18) + 
...
g2
g1
ggsave("~/Supp_External_data_objects/pdf/GSE126078.pdf", width = 8.5, height = 4)
...
leg1 + leg2
ggsave("~/Supp_External_data_objects/pdf/GSE126078_leg.pdf", width = 20, height = 2, units = "cm")
...
Neuroendocrine Differentiation in Prostate Cancer Requires ASCL1
```{r}
counts <- fread("~/Supp_External_data_objects/GSE264598/GSE264598_sc_obj_sctran_combined_counts.tsv.gz")
meta <- fread("~/Supp_External_data_objects/GSE264598/GSE264598_sc_obj_sctran_combined_annot_df.tsv.gz")
...
Dynamic_prostate_cancer_transcriptome
```{r}
...
# atlas <- readRDS("~/Supp_External_data_objects/pcatlas_dataset/atlas_vst_normalized.Rds")
# head(atlas_meta)
...
# saveRDS(atlas, "~/Supp_External_data_objects/pcatlas_dataset/atlas_vst_normalized.Rds")
...
atlas <- readRDS("~/Supp_External_data_objects/pcatlas_dataset/atlas.Rds")
...
```{r}
living_data <- readRDS("~/Supp_External_data_objects/living_data.Rds")
living_tumour_dataset <- readRDS("~/Supp_External_data_objects/living_tumour_dataset.Rds")
...
saveRDS(plotdf, "~/Supp_External_data_objects/GSE41192_living_tumor/plotdf.Rds")
...
data <- read.table(gzfile("~/Supp_External_data_objects/GSE211856/GSE211856_TPM_LNCAP.xenograft_ccnelson.txt.gz"), header = TRUE, sep = "\t")
head(data)
